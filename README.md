...
No Restrictions LMAO . Download queuing is planned, but not implemented yet.

### Warning

There is very little preventing users from using this to mirror pirated content
...
1. Clone the repo:
   ...
   ```bash
   git clone https://github.com/architmadankar/TorrentToGoogleDrive.git
   cd TorrentToGoogleDrive
   ```
...
10. Open Telegram, and send `/mirror https://raw.githubusercontent.com/architmadankar/TorrentToGoogleDrive/master/README.md` to the bot.
...
* `ARIA_DOWNLOAD_LOCATION`: This is the directory that aria2 will download files into, before uploading them. Make sure that there is no trailing "/" in this path. The suggested path is `/path/to/TorrentToGoogleDrive/downloads`
...
For Installing Node.js And npm 
   ...
   * url to setup the node.js repository in your sources.
